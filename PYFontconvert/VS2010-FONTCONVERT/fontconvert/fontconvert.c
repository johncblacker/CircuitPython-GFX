...
void enbit(uint8_t value) {
	...
	if(!(bit >>= 1)) {       // Advance to next bit, end of byte reached?
		...
		fwrite(&sum, sizeof(char), 1, binfile);		// output a byte of data to binary file
...
int main(int argc, char *argv[]) {
	...
	// buiild name for output binary file
	...
	// **************** build binary filename for open **********************
	// format will be:
	//		fontname + size (argv[2]) + "pt7b.bin" - in string format
	//			for use in fopen() functiion
	//		
	//
	strcpy(pinfn, fontName);	// pick up fontname
	//strcat(pinfn, argv[2]);	// append size
	strcat(pinfn, ".bin");	// append filename extension = .bin
	//strcpy(poutfn, pinfn);	// copy to binfilename
	// *
	// ***************************** open binary file ***********************
	// *


	binfile = fopen(pinfn, "wb");

	if (binfile == NULL) {
		err = 3;
		fprintf(stderr, "Binary open failure: %d", err);
		return err;
	}
	//*
	// **********************************************************************
